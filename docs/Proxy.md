...
**host** | **str** | Specify the fully qualified domain name, or ip address, of the proxy server.  | [optional] 
**password** | **str** | Specify the password used to authenticate with the proxy server, if required.  | [optional] 
**scheme** | **str** | The scheme accepted by the proxy server. Specify one of HTTP and HTTPS.  | [optional] 
**enabled** | **bool** | Flag to indicate if proxy is enabled. When set to true, a scheme, host and port must be provided.  | 
**port** | **int** | Specify the port of the proxy server. | [optional] 
